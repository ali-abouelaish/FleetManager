    ...
    try {
      ...
      if (uploadedDocuments.length > 0 && assistant) {
        ...
        if (documentsError) {
          ...
          setError(`Passenger assistant updated but failed to save documents: ${documentsError.message}`)
        } else {
          console.log(`Successfully inserted ${insertedDocs?.length || 0} document(s):`, insertedDocs)
        }
      } else if (uploadedDocuments.length > 0 && !assistant) {
        console.error('Cannot create documents: assistant data is missing')
        setError('Assistant data is missing. Cannot save documents.')
      }
      ...
      <div className="space-y-6">
        <Card className="border-l-4 border-red-500 bg-red-50">
          <CardContent className="py-4">
            <p className="text-sm text-red-700">Passenger assistant not found</p>
          </CardContent>
        </Card>
      </div>
    )
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-gray-600">
              {assistant.employees?.full_name || 'Passenger Assistant'}
            </p>
          </div>
        </div>
      </div>
            ...
            <CardContent className="pt-6 space-y-6">
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-900">
                  <strong>Employee:</strong> {assistant.employees?.full_name}
                </p>
                <p className="text-xs text-blue-700 mt-1">
                  To edit employee information, go to{' '}
                  <Link href={`/dashboard/employees/${assistant.employees?.id}/edit`} className="underline">
                    Employee Profile
                  </Link>
                </p>
              </div>
            ...
            <CardContent className="pt-6 space-y-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Add any additional notes or comments about this passenger assistant..."
                />
              </div>
            </CardContent>
          </Card>
